\datethis
@*Intro. This program is the eleventh in a series of exploratory studies by
...
It's quite different from its predecessors: Now I must bite the bullet
and implement general routines for dealing with many functions simultaneously
in a BDD base. So I need to worry about dynamic storage allocation
and garbage collection, and I also need a more complicated user interface.

In this program I try to implement simplified versions of the basic routines
that are needed in a ``large'' BDD package,
except that I don't tackle the problem of variable reordering.
...
The computation is governed by primitive commands, which can either be
read from a file or typed online (or both).
@^BDDL, a primitive language for BDD calculations@>
Such commands have the following simple syntax, where \<number> denotes
...
is kept in location |mem[b[k>>12]+(k&0xfff)]| for some base address~|b|.
...
The |nodeavail| stack is linked by its |lo| fields.
...
node* reserve_node(void) {
  ...
  if (r) o,nodeavail=node_(nodeavail->lo);
  else {
    r=nodeptr;
...
if (j>=0) {
  for (p=nodeavail;p&&j<3;o,p=(node_(p->lo))) j++;
  if (j>=3) break;
}
...
$n$ lies between $m/4$ and $3m/4$, inclusive. Thus at least
one of every four table slots is occupied, and
...
typedef struct var_struct {
  ...
  int rank; /* low rank means near the top of the BDD */
  unsigned int timestamp; /* time stamp for composition */
  int serial; /* presently used only by |math_print| */
} var;
...
@d initnewnode(p,v,l,h) oo,p->lo=addr_(l),p->hi=addr_(h),p->xref=0,@|
        oooo,p->index=((v)<<(32-logvarsize))+(gb_next_rand()>>logvarsize)
...
number of nodes allocated. Second, we can try to change the ranks
of the variables. The second alternative will be tried in a later
...
@<Double the table size and |goto restart|@>=
{
  ...
  free_node(p);
  if (verbose&256)
    printf("doubling the hash table for x%d (%d slots)\n",
           v-varhead,(newmask+1)/sizeof(addr));
...
@<Reserve new all-|NULL| pages for the bigger table@>=
{
  if (newmask>maxmask) { /* too big: can't go there */
    if (verbose&(2+256+512))
      printf("profile limit reached for x%d\n",v-varhead);
    goto cramped;
  }
...
if (newmask!=mask) {
  if (verbose&256)
    printf("downsizing the hash table for x%d (%d slots)\n",
           v-varhead,(newmask+1)/sizeof(addr));
  free-=(mask-newmask)>>2;
...
and $x_v=1$. The variables on every path have increasing rank, and no
...
@d print_node(p)
  printf("%x: (~%d?%x:%x)",id(p),varpart((p)->index),id((p)->lo),id((p)->hi))
...
@ The masking feature is useful when we want to print out only a single BDD.

@<Sub...@>=
void print_function(node *p) {
  unsigned long long savemems=mems; /* mems aren't counted while printing */
...
for (count=2,p=topsink+1;p<nodeptr;p++) if (ghost(p)->xref!=-1) {
  ...
  if (!legit(node_(p->lo)) || !legit(node_(p->hi)))
    ...
    if (node_(p->lo)>topsink && thevar(p)->rank>=thevar(node_(p->lo))->rank)
      complain("bad lo rank");
    if (node_(p->hi)>topsink && thevar(p)->rank>=thevar(node_(p->hi))->rank)
...
for (p=nodeavail;p;p=node_(p->lo)) {
  if (!superlegit(p))
...
for (v=varhead;v<topofvars;v++) if (v->proj) {
  if (!superlegit(node_(v->proj)))
    printf("! illegal projection function for x%d\n",v-varhead);
  else ghost(v->proj)->index++;
  if (v->repl) {
    if (!legit(node_(v->repl)))
      printf("! illegal replacement function y%d\n",v-varhead);
...
for (v=varhead;v<topofvars;v++) if (v->proj) {
  for (k=0;k<=v->mask>>logpagesize;k++)
    if (badpage(page_(v->base[k])))
      printf("! bad page base %x in unique table for x%d\n",
  ...
  for (k=count=0;k<v->mask;k+=sizeof(addr)) {
    ...
    if (!p) count++;
      ...
      if (addr__((size_t)(v->base[k>>logpagesize]+(k&pagemask))
                         -(size_t)mem+(size_t)smem)!=sanitycount)
        printf("! extra node %x in unique table for x%d\n",id(p),v-varhead);
      if (!superlegit(p))
        printf("! illegal node %x in unique table for x%d\n",id(p),v-varhead);
...
if (oo,vf->rank<vg->rank) v=vf;
  ...
  if (o,node_(g->lo)==botsink) {
    oo,r=constrain_rec((vf==v? o,node_(f->hi): f),node_(g->lo));
    goto shortcut;
  }
  if (o,node_(g->lo)==botsink) {
    oo,r=constrain_rec((vf==v? o,node_(f->hi): f),node_(g->lo));
...
if (vg->rank>v->rank) r=unique_find(v,r0,r1);
else {
...
node* diff_rec(node*f, node*g) {
  ...
  v=thevar(f);
  vg=thevar(g);
  if (oo,v->rank>vg->rank) return oo,botsink->xref++,botsink;
...
if (vg->rank>v->rank) r=unique_find(v,r0,r1);
  ...
  if (curop==10) r=mux_rec(r0,botsink,r1); /* $f\(N)g=\bar r_0\land r_1$ */
  else r=mux_rec(r1,botsink,r0); /* $f\(Y)g=r_0\land\bar r_1$ */
  deref(r0);@+deref(r1); /* we're done with |r0| and |r1| */
}
gotr:@+if (r) {
  if ((verbose&128)&&(v<tvar))
    printf("   %x=%x%c%x (level %d)\n",
            id(r),id(f),binopname[curop][0],id(g),v-varhead);
...
if (g==botsink) vg=topofvars;@+else {
  vg=thevar(g);@+if (oo,v->rank>vg->rank) v=vg;
}
if (h==topsink) vh=topofvars;@+else {
  o,vh=thevar(h);@+if (o,v->rank>vh->rank) v=vh;
}
...
vg=thevar(g);
o,vh=thevar(h);
if (oo,vf->rank<vg->rank) v=vf;@+else v=vg;
if (o,v->rank>vh->rank) v=vh; /* choose the top variable, |v| */
...
if (vh->rank>v->rank) r=unique_find(v,r0,r1);
else {
  r=or_rec(r0,r1); /* existential quantification happens here */
  deref(r0);@+deref(r1); /* we're done with |r0| and |r1| */
}
...
if (q==NULL) { /* cancel a previous replacement */
  ...
  else timestamp=timestamp-timestamp_changed, timestamp_changed=0;
}@+else {
  oo,q->xref++;
...
node* binary_top(int curop, node*f, node*g) {
  ...
  if (verbose&2)
     printf("beginning to compute %x %c %x:\n",
          id(f),binopname[curop][0],id(g));
  cacheinserts=0;
  while (1) {
    switch (curop) {
...
case 10: case 12: r=yes_no_rec(curop,f,g);@+break; /* $f\(Y)g$ or $f\(N)g$ */
...
default: fprintf(stderr,"This can't happen!\n");@+exit(-69);
  ...
  if (verbose&(1+2))
    printf(" %x=%x%c%x (%llu mems, %llu rmems, %llu zmems)\n",@|
                id(r),id(f),binopname[curop][0],id(g),
...
node* ternary_top(int curop, node*f, node*g, node*h) {
  ...
  unsigned long long oldmems=mems, oldrmems=rmems, oldzmems=zmems;
  if (verbose&2)
     printf("beginning to compute %x %c %x %c %x:\n",
          id(f),ternopname1[curop-16][0],id(g),ternopname2[curop-16][0],id(h));
  cacheinserts=0;
  while (1) {
    switch (curop) {
...
default: fprintf(stderr,"This can't happen!\n");@+exit(-69);
  ...
  if (verbose&(1+2))
    printf(" %x=%x%c%x%c%x (%llu mems, %llu rmems, %llu zmems)\n",@|
       id(r),id(f),ternopname1[curop-16][0],id(g),ternopname2[curop-16][0],
...
case 'f': @<Parse and execute an assignment to $f_j$@>;@+continue;
...
case 'S': show_stats();@+continue;
case 't': @<Reset |tvar|@>;@+continue;
case 'v': getk;@+verbose=k;@+continue;
case 'V': verbose=-1;@+continue;
case 'y': @<Parse and execute an assignment to $y_j$@>;@+continue;
default: reporterror;
}
...
@<Parse and execute an assignment to $f_j$@>=
getkf;@+lhs=k;
passblanks;
if (*c++!='=') reporterror;
...
passblanks;
switch (*c++) {
case 'x': getkv;@+p=projection(k);@+break;
...
passblanks;
switch (*c++) {
case 'x': getkv;@+q=projection(k);@+break;
...
if (curop<=maxbinop) r=NULL;
  ...
  passblanks;
  switch (*c++) {
case 'x': getkv;@+r=projection(k);@+break;
...
@ @<Parse and execute an assignment to $y_j$@>=
getkv;@+v=&varhead[k];
...
void math_print(node *p) {
  ...
  if (!p) return;
  outcount++;
  sprintf(buf,"/tmp/bdd11-out%d.m",outcount);
  outfile=fopen(buf,"w");
  if (!outfile) {
...
@ @<Generate Mathematica outputs for variable |v|@>=
{
  ...
  if (t) v->serial=++s; /* this many levels exist below |v| */
}

@ @<Generate a Mathematica line for node |q|@>=
fprintf(outfile,"g%x=Expand[",id(q));
r=node_(q->lo);
ss=(r<=topsink?0:thevar(r)->serial);
if (ss<s) {
