...
The new approach is based on QBDDs (quasi-reduced BDDs),
...
The given function is specified explicitly by generating its QBDD.
...
@* QBDD representation. The quasi-reduced binary decision decision
...
@ Once the |node| arrays exist, we can set of the initial QBDD.
...
@<Compute the |dep| fields of the initial QBDD@>;

@ For diagnostic purposes, I might want to pretty-print the current QBDD.
...
@*Reduction. Restructings of the QBDD are implemented with the help
...
{
  if (q>=worksize) {
    fprintf(stderr,"Sorry, level %d of the QBDD is too big (worksize=%d)!\n",
             k,worksize);
    exit(-3);
  }
  if (lo!=hi)
...
@ @<Glob...@>=
int clone[1<<21]; /* the new node addresses */

@ Let's illustrate |reduce| by using it to tidy up the initial QBDD.
...
QBDD in which the elements of~$X$ appear on the levels 0 through~$k-1$.
...
Notice that every QBDD has information about $n$ different subsets
...
all the QBDDs we need, using a relatively short sequence of jump-ups.
...
@<Jump up from |kk| to |jj|@>;
for (k=jj+1;k<=kk;k++)
  @<Gather statistics from level |k| of the current QBDD@>;

@ We also need to gather statistics from the initial QBDD.

@<Init...@>=
@<Gather statistics from level 0 of the current QBDD@>;
for (k=1;k<nnn;k++)
  @<Gather statistics from level |k| of the current QBDD@>;
...
@ At this point we have the initial QBDD, with |map[j]=j| for all |j|.
