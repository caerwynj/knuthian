...
offsets for the piece named \.2 in the silly example are
...
stored as two 32-bit quantities |0x12000000| and |0x00022000|.
...
@ @<Sub...@>=
void print_big(int hi, int lo)
{
  printf("%.15g",((double)hi)*4294967296.0+(double)lo);
}
@#
void print_bigx(int hi, int lo)
{
...
The value returned is 0 if the |board| isn't new, otherwise~1.
...
@ When we encounter a new configuration, we print it if it is the
...
@ The recursive calls here essentially amount to a {\it depth-first\/} search
of the places to which the piece can move.

Local variables |s| and |t| need not be preserved across the recursive
call, but I don't expect the compiler to recognize that fact.
...
Elements |inx[k]| through |inx[k+1]-1| of this array are known to be
simultaneously either in or out of the ideal, based on the decision
made on level~|k| of a backtrack tree.

The basic invariant relation is that we could obtain an ideal by either
excluding or including all elements of index $\ge|inx[k]|$ in |perm|,
for $0\le k\le l$, where $l$ is the current level. This property holds
